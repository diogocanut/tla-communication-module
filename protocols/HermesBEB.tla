...
VARIABLES
    invChan,         \* BEB channel carrying INV messages
    valChan,         \* BEB channel carrying VAL messages
...
HInit ==
    /\ invChan          = BEB!Channel({InvGroup}, H_NODES)
    /\ valChan          = BEB!Channel({ValGroup}, H_NODES)
...
h_send_inv(n, newVersion, newTieBreaker) ==
    ...
    IN  \E nextInv \in BEB!Broadcast(invChan, InvGroup, n, m) :
            invChan' = nextInv
...
HSendVals(n) ==
    ...
    /\ LET m == [type       |-> "VAL",
       ...
       IN  \E nextVal \in BEB!Broadcast(valChan, ValGroup, n, m) :
               valChan' = nextVal
...
HRcvInv(n) ==
    /\ BEB!HasMessage(invChan, InvGroup, n)
    /\ \E m \in BEB!Messages(invChan, InvGroup, n) :
        /\ m.type    = "INV"
        /\ m.epochID = epochID
        /\ m.sender /= n
        /\ invChan' = BEB!Deliver(invChan, InvGroup, n, m)
...
HRcvVal(n) ==
    /\ BEB!HasMessage(valChan, ValGroup, n)
    /\ \E m \in BEB!Messages(valChan, ValGroup, n) :
        /\ m.type = "VAL"
        /\ nodeState[n] /= "valid"
        /\ equalTS(m.version, m.tieBreaker,
                   nodeTS[n].version, nodeTS[n].tieBreaker)
        /\ valChan'   = BEB!Deliver(valChan, ValGroup, n, m)
        /\ nodeState' = [nodeState EXCEPT ![n] = "valid"]
        /\ UNCHANGED <<invChan, ackLink, nodeTS, nodeLastWriter, nodeLastWriteTS,
                       nodeRcvedAcks, nodeWriteEpochID, aliveNodes, epochID>>
...
HNodeFailure(n) ==
    ...
    /\ BEB!CanCrash(invChan)
    /\ aliveNodes'    = aliveNodes \ {n}
    /\ epochID'       = epochID + 1
    /\ nodeRcvedAcks' = [k \in H_NODES |-> {}]
    /\ invChan'       = BEB!Crash(invChan, n)
    /\ valChan'       = BEB!Crash(valChan, n)
